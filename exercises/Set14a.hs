...
greetText s = T.pack "Hello, " <> (if T.length s > 15 then T.take 15 s <> T.pack "..." else s) <> T.pack "!"
...
longestRepeat s
    | T.null s = 0
    | otherwise = maximum . map T.length . T.group $ s
...
byteRange s = if mns == 255 then 0 else mxs - mns
    where (mns, mxs) = B.foldl (\(mn, mx) x -> (min mn x, max mx x)) (255, 0) s
...
pingpong b = BL.concat $ cycle [BL.fromStrict b, BL.fromStrict revb]
    where revb = B.reverse b
