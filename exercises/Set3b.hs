...
buildList start count end = if count == 0 then [end]
...
sums :: Int -> [Int]
sums i = sums' i 1 0
    where sums' n d s = if n == 0 then []
                        else s + d : sums' (n - 1) (d + 1) (s + d)
...
indexDefault xs i def = if i < 0 then def
                        else case xs of
                            [] -> def
                            (x:xs) -> if i == 0 then x
...
merge xs [] = xs
merge [] ys = ys
merge (x:xss) (y:yss) = if x <= y then x:merge xss (y:yss)
                                  else y:merge (x:xss) yss
...
mymaximum bigger initial (x:xs) = mymaximum bigger upd xs
    where upd = if bigger initial x then initial else x
