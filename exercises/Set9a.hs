...
countValid = sum . map valid
  where valid s = if (s !! 2 == s !! 4) || (s !! 3 == s !! 5) then 1 else 0
------------------------------------------------------------------------------
...
sumSuccess = foldl sumEither (Left "no data")
  where sumEither (Left _) (Left _) = Left "no data"
        sumEither (Left _) (Right x) = Right x
        sumEither (Right x) (Left _) = Right x
...
open s (Lock pwd b)
  | s == pwd = Lock pwd True
  | otherwise = Lock pwd b

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (Lock pwd _) = Lock pwd False

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode newPwd (Lock pwd b) = if b then Lock newPwd True else Lock pwd False
...
permute p xs = map fst $ sortBy (comparing snd) $ zip xs p
