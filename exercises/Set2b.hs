...
leftpad s n = (replicateString (n - length s) " ") ++ s
    where replicateString n s = concat $ replicate n s
...
countdown n = "Ready! " ++ countdown' n ++ " Liftoff!"
    where countdown' n = unwords [show p ++ "..." | p <- [n, n - 1 .. 1] ]
...
smallestDivisor n
    ...
    | otherwise = smallestDivisor' n 2
        where smallestDivisor' n p
                | n `mod` p == 0 = p
                | otherwise = smallestDivisor' n (p + 1)
