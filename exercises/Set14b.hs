...
import Database.SQLite.Simple (Connection, Only (Only), Query (..), execute, execute_, fromOnly, open, query, query_)
import Mooc.Todo
...
deposit db name amount = do
  execute db depositQuery (name, amount)
  return ()
...
balance db name = do
  rows <- query db balanceQuery (Only name)
  return (sum (map fromOnly rows))
...
parseCommand [a, b] = if a == T.pack "balance" then Just (Balance b) else Nothing
parseCommand [a, b, c] =
  if a == T.pack "deposit"
    then case parseInt c of
      Just i -> Just (Deposit b i)
      Nothing -> Nothing
    else
      if a == T.pack "withdraw"
        then case parseInt c of
          Just i -> Just (Withdraw b i)
          Nothing -> Nothing
        else Nothing
...
server db request respond = case parseCommand (pathInfo request) of
  Just command -> do
    response <- perform db (Just command)
    respond $ responseLBS status200 [] (encodeResponse response)
  Nothing -> respond $ responseLBS status200 [] (encodeResponse (T.pack "ERROR"))
