...
joinToLength len ls = [s1 ++ s2 | s1 <- ls, s2 <- ls, length (s1 ++ s2) == len]
...
sumRights = foldr applyRight 0
    where applyRight (Right x) acc = x + acc
          applyRight _ acc = acc
...
interpreter = interpret 0 0
    where interpret x y [] = []
          interpret x y (cmd:cmds) = case cmd of
              "up" -> interpret x (y+1) cmds
              "down" -> interpret x (y-1) cmds
              "left" -> interpret (x-1) y cmds
              "right" -> interpret (x+1) y cmds
