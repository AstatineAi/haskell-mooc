...
add a (Set xs) = Set (insertOrdered a xs)
  where insertOrdered a [] = [a]
        insertOrdered a (x:xs)
          | a < x = a : x : xs
          | a == x = x : xs
...
passwordAllowed pwd (ContainsSome cs) = any (`elem` cs) pwd
passwordAllowed pwd (DoesNotContain cs) = not (any (`elem` cs) pwd)
...
data Arithmetic = Operation Operator Arithmetic Arithmetic | Literal Integer
  deriving Show
...
render (Operation op a b) = "(" ++ render a ++ show op ++ render b ++ ")"
