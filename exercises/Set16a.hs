...
frequenciesProp freq input =
  conjoin
    [ sumIsLength (getNonEmpty input) (freq (getNonEmpty input)),
      inputInOutput (getNonEmpty input) (freq (getNonEmpty input)),
      outputInInput (getNonEmpty input) (freq (getNonEmpty input))
    ]
...
instance Arbitrary Arg where
  arbitrary = do
    n <- choose (0, 10)
    v <- elements "abcxyz"
    oneof [return (Number n), return (Variable v)]

instance Arbitrary Expression where
  arbitrary = do
    arg1 <- arbitrary
    arg2 <- arbitrary
    oneof [return (Plus arg1 arg2), return (Minus arg1 arg2)]
