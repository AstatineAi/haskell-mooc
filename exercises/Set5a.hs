...
import Mooc.Todo
import Distribution.PackageDescription (BuildType(Make))
...
data Person = MakePerson Int String
  deriving Show

-- fred is a person whose name is Fred and age is 90
fred :: Person
fred = MakePerson 18 "Fred"

-- getName returns the name of the person
getName :: Person -> String
getName (MakePerson _ name) = name

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (MakePerson age _) = age

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name (MakePerson age _) = MakePerson age name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (MakePerson _ name) = MakePerson age name
...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix x y) = [ (a + b) / 2 | (a, b) <- zip xs ys ]
    where xs = rgb x; ys = rgb y
rgb (Invert x) = [1 - a | a <- xs]
    where xs = rgb x
