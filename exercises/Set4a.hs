...
distinct xs = length [x | x <- xs, y <- xs, x == y] == length xs
...
middle x y z
    | x < y = if y < z then y else max x z
    | otherwise = if x < z then x else max y z
...
longest :: (Ord a) => [[a]] -> [a]
longest = foldr (\x y -> if length x > length y || (length x == length y && head x < head y) then x else y) []
...
incrementKey km = map (\(k, v) -> if k == km then (k, v + 1) else (k, v))
...
freqs = foldr (Map.alter (\v -> case v of Just n -> Just (n + 1); Nothing -> Just 1)) Map.empty
...
transfer from to amount bank
    ...
    | otherwise = Map.adjust (\v -> v - amount) from $ Map.adjust (+ amount) to bank
...
maxIndex arr = fst $ foldl1 pickMax idList
    where idList = Array.assocs arr
          pickMax :: (Ix i, Ord a) => (i, a) -> (i, a) -> (i, a)
          pickMax (mxi, mxv) (i, v) = if v > mxv then (i, v) else (mxi, mxv)
