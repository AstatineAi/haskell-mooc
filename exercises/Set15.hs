...
validateDiv x y = check (y /= 0) "Division by zero!" (div x y)
...
validateAddress streetName streetNumber postCode = checkLength *> checkNumber *> checkPostCode
  where
    checkLength = check (length streetName <= 20) "Invalid street name" address
    checkNumber = check (all isDigit streetNumber) "Invalid street number" address
    checkPostCode = check (length postCode == 5 && all isDigit postCode) "Invalid postcode" address
    address = Address streetName streetNumber postCode
...
twoPersons ::
  ...
  f Bool ->
  f [Person]
twoPersons name1 age1 employed1 name2 age2 employed2 = liftA2 (\p1 p2 -> [p1, p2]) person1 person2
  where
...
boolOrInt s = boolResult <|> intResult
  where
    boolResult = case readMaybe s of
      Just b -> pure (Left b)
      Nothing -> invalid "Not a Bool"
    intResult = case readMaybe s of
      Just i -> pure (Right i)
...
normalizePhone s = checkLength *> checkDigits
  where s' = filter (/= ' ') s
        checkLength = check (length s' <= 10) "Too long" s'
        checkDigits = traverse checkDigit s'
...
parseExpression rawExpr = if length wordsExpr /= 3
  then invalid ("Invalid expression: " ++ rawExpr)
  else expr
  where
    wordsExpr = words rawExpr
    checkLength = check (length wordsExpr == 3) ("Invalid expression: " ++ rawExpr) ()
    parseOp = case wordsExpr !! 1 of
      "+" -> pure Plus
      "-" -> pure Minus
      _ -> invalid ("Unknown operator: " ++ words rawExpr !! 1)
    parseArg1 = parseArg $ head wordsExpr
    parseArg2 = parseArg $ last wordsExpr
    -- discard latter result if checkLength fail
    expr = parseOp <*> parseArg1 <*> parseArg2

parseArg :: String -> Validation Arg
parseArg arg = checkNumber <|> checkVariable
  where
    checkNumber = case readMaybe arg of
      Just n -> pure (Number n)
      Nothing -> invalid ("Invalid number: " ++ arg)
    checkVariable = if length arg == 1 && isAlpha (head arg)
      then pure (Variable (head arg))
      else invalid ("Invalid variable: " ++ arg)
...
instance Applicative Priced where
  ...
  liftA2 f (Priced price1 value1) (Priced price2 value2) = Priced (price1 + price2) (f value1 value2)
...
myFmap f x = myLiftA2 (\x _ -> f x) x (myPure ())
